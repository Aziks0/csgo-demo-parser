...
use crate::messages::{csvcmsg_class_info, CSVCMsg_SendTable};
use crate::parser::packet::message::{Message, SvcMessage};
...
impl DataTables {
    ...
    pub(crate) fn try_new(reader: &mut CodedInputStream) -> Result<Self> {
        ...
        loop {
            ...
            trace!(?message);
            match message {
                Message::Svc(SvcMessage::SendTable(send_table)) => {
                ...
                _ => {
                    return Err(DataTablesParsingError::NotASendTable(message).into());
                }
            }
        }
        ...
        for _ in 0..class_count {
            ...
            server_class.set_class_name(reader.read_string_to_terminator(256)?.into());
            server_class.set_data_table_name(reader.read_string_to_terminator(256)?.into());
